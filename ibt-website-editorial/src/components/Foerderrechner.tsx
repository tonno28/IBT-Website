...
export default function Foerderrechner() {
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      {step === 0 && (
        <div>
          <h2 className="text-base font-semibold text-zinc-primary mb-4">
                ...
                <div>
                  <div
                    className={`font-semibold text-sm ${
                      typ === key ? "text-zinc-primary" : "text-zinc-secondary"
                    }`}
                  >
                    {val.label}
                  </div>
                  <div className="text-xs text-zinc-muted mt-0.5">{val.desc}</div>
                </div>
              </button>
            ))}
          </div>
      ...
      {step === 1 && m && (
        <div>
          <h2 className="text-base font-semibold text-zinc-primary mb-5">
            Gebäudedetails
          </h2>

          <div className="mb-6">
            <label className="block text-sm text-zinc-secondary mb-3">
          ...
          <div className="mb-6">
            <label className="block text-sm text-zinc-secondary mb-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-3 pr-10 bg-bg-card border border-zinc-border rounded-xl text-zinc-primary font-mono text-base focus:outline-none focus:border-amber transition-colors"
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-hint text-sm">
                €
              </span>
            </div>
          </div>
      ...
      {step === 2 && m && (
        <div>
          <h2 className="text-base font-semibold text-zinc-primary mb-1.5">
            Förderboni prüfen
          </h2>
              ...
              >
                <div
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${
                    aktiveBoni[b.id]
                      ? "bg-amber border-amber text-onAccent"
                      : "border-zinc-borderHover"
                  }`}
                >
                  {aktiveBoni[b.id] && (
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-zinc-muted mt-0.5 leading-relaxed">{b.desc}</p>
                </div>
              </button>
            ))}
          </div>
          ...
          {/* Live rate */}
          <div className="bg-bg-card border border-zinc-border rounded-xl p-4 mb-5">
            <div className="text-xs text-zinc-muted mb-1">Aktueller Fördersatz</div>
            <div className="text-3xl font-bold font-mono text-amber">{currentRate} %</div>
            <div className="text-xs text-zinc-hint mt-0.5">
      ...
      {step === 3 && result && (
        <div>
          <div className="text-center mb-6">
            ...
            <div className="text-5xl font-bold font-mono text-teal-light">
              {fmt(result.foerderBetrag)}
            </div>
            <div className="text-sm text-zinc-secondary mt-1">
          ...
          <div className="card-base p-5 mb-4 space-y-3">
            ...
            <div className="border-t border-zinc-border pt-3 space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-zinc-muted">Basis-Fördersatz</span>
                <span className="text-zinc-secondary">{m?.basis} %</span>
              </div>
              {result.bonusProzent > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-zinc-muted">Boni</span>
                  <span className="text-amber font-semibold">+{result.bonusProzent} %</span>
                </div>
              )}
              <div className="flex justify-between text-sm font-semibold">
                <span className="text-zinc-muted">Gesamt-Fördersatz</span>
                <span className="text-amber">{result.foerderSatz} %</span>
              </div>
            </div>
            ...
            <div className="border-t border-zinc-border pt-3 space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-zinc-muted">+ Baubegleitung (50 %)</span>
                <span className="text-teal-light font-mono">bis {fmt(result.bbFoerder)}</span>
              </div>
              <div className="flex justify-between font-bold">
                <span className="text-zinc-primary">Maximale Gesamtförderung</span>
          ...
          <div className="card-base p-5 mb-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-zinc-muted">Geschätzter Eigenanteil</div>
                <div className="text-2xl font-bold font-mono text-zinc-primary">
                  {fmt(Math.max(0, (parseFloat(investition) || 0) - result.foerderBetrag))}
                </div>
              </div>
              <div className="text-right text-sm">
                <div className="text-zinc-hint">statt {fmt(parseFloat(investition) || 0)}</div>
